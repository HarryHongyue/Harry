...
interface OmnigentShowcaseProps {
  project: Project;
}
...
const OmnigentShowcase: React.FC<OmnigentShowcaseProps> = ({ project }) => {
  const { currentLanguage } = useLanguage();
  const visitUrl = project.websiteUrl ?? project.demoUrl ?? '/projects/omnigent';
  ...
  const value = l(`The website presents Omnigent as a practical partner for companies that need clearer systems, better operational visibility, and a structured path from first enquiry to scoped delivery.`, `网站将 Omnigent 呈现为务实的企业合作伙伴，帮助客户获得更清晰的系统、更好的运营可视化，以及从初次咨询到明确交付范围的完整路径。`, `De website positioneert Omnigent als praktische partner voor bedrijven die duidelijkere systemen, betere operationele zichtbaarheid en een gestructureerd pad van aanvraag naar scoped delivery nodig hebben.`);
  ...
  const vision = l(`The vision is to make Omnigent a credible bridge between software and real operations: a site that can sell custom systems, logistics digitisation, warehouse implementation support, and Excel-to-system transformation without sounding like a generic agency.`, `愿景是让 Omnigent 成为软件与真实运营之间可信的桥梁：它能销售定制系统、物流数字化、仓库实施支持和 Excel 到系统的转型，但不显得像泛泛的软件外包公司。`, `De visie is Omnigent positioneren als geloofwaardige brug tussen software en echte operations: custom systems, logistieke digitalisering, warehouse implementation support en Excel-to-system transformatie verkopen zonder generiek agencygevoel.`);
  ...
  return (
    <>
      <section className="section-shell omnigent-website-hero">
        <div className="omnigent-website-hero__inner">
          <div className="omnigent-website-hero__content">
            <h1 className="omnigent-website-hero__title">Omni<span>gent</span></h1>
            <p className="omnigent-website-hero__description">{pickText(currentLanguage, project.longDescription)}</p>
            <div className="omnigent-website-hero__actions">
              <a href={visitUrl} className={`${neoButtonClass('primary')} omnigent-website-button`} target="_blank" rel="noreferrer"><ExternalLink size={18} />{currentLanguage === 'zh' ? '访问网站' : currentLanguage === 'nl' ? 'Website bezoeken' : 'Visit Website'}</a>
            </div>
          </div>
          <div className="omnigent-website-hero__visual" aria-hidden="true">
            <NeoCard className="omnigent-website-browser-card">
              <div className="omnigent-website-browser-card__bar"><span /><span /><span /><strong>{project.slug}</strong></div>
              <div className="omnigent-website-browser-card__hero"><BriefcaseBusiness size={42} /><h3>{project.englishName}</h3><p>{pickLocal(currentLanguage, focus)}</p></div>
              <div className="omnigent-website-browser-card__grid">{structure.map((section) => <div key={section}><PanelsTopLeft size={18} /><span>{section}</span></div>)}</div>
            </NeoCard>
          </div>
        </div>
      </section>

      <section className="section-shell omnigent-website-section">
        <h2 className="omnigent-website-section__title">{currentLanguage === 'zh' ? '网站定位与愿景' : currentLanguage === 'nl' ? 'Positionering en visie' : 'Positioning & Vision'}</h2>
        <p className="omnigent-website-section__subtitle">{pickText(currentLanguage, project.description)}</p>
        <div className="omnigent-website-focus-grid">
          <NeoCard hoverable className="omnigent-website-focus-card"><Sparkles size={24} /><h3>{currentLanguage === 'zh' ? '核心价值' : currentLanguage === 'nl' ? 'Kernwaarde' : 'Core Value'}</h3><p>{pickLocal(currentLanguage, value)}</p></NeoCard>
          <NeoCard hoverable className="omnigent-website-focus-card"><Wrench size={24} /><h3>{currentLanguage === 'zh' ? '交付亮点' : currentLanguage === 'nl' ? 'Delivery Highlights' : 'Delivery Highlights'}</h3><p>{pickLocal(currentLanguage, implementation)}</p></NeoCard>
          <NeoCard hoverable className="omnigent-website-focus-card"><Telescope size={24} /><h3>{currentLanguage === 'zh' ? '项目愿景' : currentLanguage === 'nl' ? 'Projectvisie' : 'Project Vision'}</h3><p>{pickLocal(currentLanguage, vision)}</p></NeoCard>
        </div>
      </section>

      <section className="section-shell omnigent-website-section">
        <h2 className="omnigent-website-section__title">{currentLanguage === 'zh' ? '功能与内容模块' : currentLanguage === 'nl' ? 'Functies en contentmodules' : 'Features & Content Modules'}</h2>
        <div className="omnigent-website-feature-grid">{modules.map((item) => <NeoCard key={item.title.en} hoverable className="omnigent-website-feature-card"><h3>{pickLocal(currentLanguage, item.title)}</h3><p>{pickLocal(currentLanguage, item.description)}</p></NeoCard>)}</div>
      </section>

      <section className="section-shell omnigent-website-section">
        <h2 className="omnigent-website-section__title">{currentLanguage === 'zh' ? '技术栈与演进方向' : currentLanguage === 'nl' ? 'Techstack en evolutie' : 'Tech Stack & Evolution'}</h2>
        <NeoCard className="omnigent-website-tech-card">
          <ProjectTechStackCards techStackIds={project.techStackIds} locale={currentLanguage} />
          <div className="omnigent-website-roadmap"><h3>{currentLanguage === 'zh' ? '维护与演进方向' : currentLanguage === 'nl' ? 'Onderhoud en evolutie' : 'Maintenance & Evolution'}</h3><ul>{maintenance.map((item) => <li key={item.en}>{pickLocal(currentLanguage, item)}</li>)}</ul></div>
        </NeoCard>
      </section>
    </>
  );
...
export default OmnigentShowcase;
